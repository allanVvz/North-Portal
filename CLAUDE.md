...
**Never junction a worktree's `node_modules` to the main checkout's.** Running agents in parallel `git worktree`s is fine, but linking `node_modules` (`mklink /J`, `New-Item -ItemType Junction`) to the main tree's copy is a trap on Windows: `git worktree remove --force` walks *through* the junction and deletes the **target**, wiping the main `node_modules`. The symptom is every command dying with `'vitest' não é reconhecido...`, which points nowhere near the cause. Either `npm ci` inside each worktree, or delete the junction before removing the worktree. Related: worktrees live under `.claude/worktrees/` — *inside* the repo — so vitest globs their copies too and reports hundreds of duplicate "failing" files. Remove the worktrees before running the suite, or the 3 real failures drown in the noise.
