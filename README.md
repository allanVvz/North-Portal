...
- **Plano de Acao é um card real** (`kind = plano_acao`), nao mais "qualquer card com `client_visible`". Outras tarefas se vinculam a ele pela coluna `tasks.plan_id` (self-FK). O portal "Plano de Acao" mostra os `plano_acao` client-visible, com faixa de datas (`start_date`/`end_date`) e progresso rollado.
...
- **Vinculo plano ↔ atividades, nos dois sentidos**: toda tarefa tem o atributo **Plano de Ação** (dropdown no modal e no painel, grava `plan_id`); e o modal de um `plano_acao` tem a secao **"Atividades do plano"** que lista os membros (com ✕ para desvincular) e um dropdown "+ Vincular atividade existente".
...
- **Um revisor nunca é um cliente antes da Aprovação.** `lib/supabase.ts` tem `listAdminReviewers()` (Revisão) e `listClientReviewerCandidates(clientId)` (Aprovação+) em vez de uma lista única — `TaskModal`/`TaskDetailPanel` trocam de lista sozinhos com base no status do card (`reviewerStageFor` em `kanbanShared.ts`) e limpam o `reviewer_id` ao cruzar essa fronteira.
...
- **Só gerente aprova/reabre** — o mesmo endpoint rejeita qualquer mudança para/de `aprovado` se `session.level !== "gerente"` (editor só comenta/ajusta). `/api/admin/metrics/[id]` (registrar métricas) usa o mesmo gate via `requireAdminManager()`.
- **Um cliente gerente aprova por qualquer `usuario` da mesma empresa** — a policy RLS `tasks client approve own` (`20260706000007`) libera `reviewer_id = auth.uid() OR is_manager()`; o mesmo vale na API (`/api/client/[slug]/tasks/[id]`) e na Feedbacks page (`isManager` em `PortalPaged.tsx`), que mostra "Aprovando como gerente da conta" quando não é o revisor original.
...
Contas de teste (`SenhaForte123!`): `admin@north.com` (gerente), `editor@north.com` (editor), `cliente@karpinski.com` (gerente do cliente), `colaborador@karpinski.com` (usuário do cliente).
