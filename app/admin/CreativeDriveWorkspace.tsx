...
export default function CreativeDriveWorkspace({ taskId, targets, summaries, initialAssetId, initialTab = "raw", selectedAssetIds, onSelectedAssetIdsChange, onChanged, onBack, onClose }: {
  ...
  async function assignRaw(targetId: string, rawIds = selectedRawIds) {
    ...
    try {
      ...
      setSelectedRawIds([]);
      setRawTargetId(targetId);
      setRawFilter("pending");
  ...
  return <div className="kb-modal-backdrop" onClick={onClose}><div className="tm tm-lg docprev-tm creative-drive-modal" onClick={(event) => event.stopPropagation()}>
    ...
    <div className="tm-layout"><div className="tm-main creative-drive-main" ref={main}>
      ...
      {targets.length ? <section className="tm-box docprev-cellbox" ref={targetRail}><p className="tm-box-label">Pastas dos Criativos</p><p className="admin-sub">Arraste brutos para a pasta, ou selecione as miniaturas e toque nela.</p><div className="creative-drive-targets">{targets.map((target) => <div key={target.id} className={`creative-drive-target${target.id === activeTaskId ? " on" : ""}`}
        ...
        <button type="button" className="creative-drive-target-assign" disabled={Boolean(busy)} onClick={() => { if (activeTab === "raw" && selectedRawIds.length) void assignRaw(target.id); else chooseRawTarget(target.id); }} aria-label={`${activeTab === "raw" && selectedRawIds.length ? "Classificar brutos em" : "Ver brutos pendentes para"} ${target.title}`}><span aria-hidden>▣</span><span><b>{target.title}</b><small>{target.id === activeTaskId ? classifiedAssets.length : targetSummaries.find((summary) => summary.creative_task_id === target.id)?.raw_links.length ?? 0} bruto(s) vinculado(s)</small></span></button>
