...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (isCreativeDeliveryKind(saved.kind) && !saved.subtype
      && (saved.title !== current.title || Boolean(payload_patch && Object.hasOwn(payload_patch, "formato")) || Boolean(planLink))) {
      try { await provisionCreativeDriveWorkspaceIfConfigured(createAdminClient(), saved.id); }
