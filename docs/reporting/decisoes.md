...
| 24/09 | **Compra do pixel vira o desfecho do objetivo de site**, como KPI `optional` (`Compras`, `Custo por compra`) em `siteKpis` e no molde de e-commerce | Cada bloco mostra o resultado do seu objetivo — mensagens mostra conversas, perfil mostra visitas — e o de site parava no clique. O pixel da CRIS registrou compras na semana de 15–21/09 e nenhum dos dois relatórios as mostrava. `optional` = só renderiza com dado no período: sem isso, conta sem pixel receberia um card "Sem integração" toda semana. Zero DITO pelo pixel continua aparecendo — é informação, diferente de ausência |
| 24/09 | **Compra do pixel + venda informada somam no funil do relatório INTERNO, como nota de fechamento, nunca como etapa** (`purchasesNote`) | Decisão do usuário: "venda e venda informada deve ser somados no funil… não deve estar no funil de conversão porque hoje não faz sentido e trackeia errado… encontre uma forma de usar positivamente sem necessariamente adicionar uma etapa". Uma etapa "Compras" depois de "Conversas" afirmaria uma cadeia inexistente e `stageGap` calcularia uma taxa falsa entre as duas |
| 24/09 | A nota de compras **nunca é um número solto**: ou diz que veio do pixel, ou mostra as duas parcelas ("4 compras · 3 pelo pixel + 1 informada pela equipe") | As duas fontes têm naturezas diferentes e precisam permanecer distinguíveis — o pixel atribui a compra à CAMPANHA; a venda que a equipe informa no Feedback é da CONTA inteira, sem atribuição. Somar só é legítimo no nível da conta, que é onde o funil vive. Por isso a soma não entra no KPI do bloco: ali afirmaria que a venda veio daquela campanha |
| 24/09 | A venda informada vem de `task_metrics` pelo período fechado, não de uma leitura da etapa de Feedback | Na primeira geração da semana o feedback ainda não aconteceu e o fecho mostra só o pixel; a soma aparece quando o tráfego é regerado depois. Ler um fato já gravado sobre o período não inverte a cascata — chamar a etapa seguinte inverteria |
| 24/09 | **O funil é a conta inteira**: visitas ao site, ao perfil e conversas somam todas as campanhas, desde que exista campanha daquele objetivo. "Mídia por objetivo" continua recortada por campanha | Pedido do usuário (CRIS: site 29→41, perfil 132→662, conversas 17→24). No Instagram todo anúncio leva ao perfil pelo @. A porta "ter o objetivo" mantém fora os fantasmas de 23/09 da Baita. O subtítulo do funil diz que ali estão todas as campanhas, senão 662 no funil e 132 no bloco parecem erro |
| 24/09 | Comentários da automação saem como **North Ai**, o mesmo nome do responsável; "Automação" segue reconhecido como autor legado (`isAutomationAuthor`) | Duas identidades para a mesma coisa no mesmo card. O autor é o que separa automação de pedido humano: comentário antigo lido como humano viraria instrução de revisão |
| 24/09 | Tabela de objetivos: coluna "Resultado" com a unidade na célula; "menor custo" só entre linhas da mesma unidade; taxa para conversa só a partir de alcance ou clique | O relatório dizia "perfil teve o menor custo por conversa" com R$ 0,51 por VISITA, e "3,41% conversaram" entre visitas e conversas |
| 24/09 | Um PDF e um comentário automático por card e por semana: regerar desliga o documento anterior (`task_id` nulo, arquivo preservado) e a RPC `replace_automatic_report_attachment` remove todos os comentários automáticos antes de inserir o novo | Cada regeração empilhava um PDF e um comentário da mesma semana. Id do comentário de manutenção é por REVISÃO: a RPC ignora id repetido |
