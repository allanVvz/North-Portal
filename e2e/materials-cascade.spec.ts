...
test("selecionar bruto classificado real mostra o Criativo vinculado no preview", async ({ page }, testInfo) => {
  ...
  await expect(tile.locator(".creative-drive-tile-eyebrow")).toContainText(title);
...
test("Captação compartilhada classifica o mesmo bruto em dois Criativos e o vínculo aparece no Criativo", async ({ page }, testInfo) => {
  ...
  await expect(tile.locator(".creative-drive-tile-eyebrow")).toContainText(taskRows[0].title);
  ...
  await expect(page.locator(`[data-classified-asset-id="${assetIdFor(ids[0], raw.id)}"] .creative-drive-tile-eyebrow`)).toContainText(taskRows[0].title);
  ...
  await page.getByRole("button", { name: "Já classificados" }).click();
  await expect(tile).toBeVisible();
  await expect(tile.locator(".creative-drive-tile-eyebrow")).toHaveCount(2);
  await expect(tile.locator(".creative-drive-tile-eyebrow").filter({ hasText: taskRows[0].title })).toHaveCount(1);
  await expect(tile.locator(".creative-drive-tile-eyebrow").filter({ hasText: taskRows[1].title })).toHaveCount(1);
  await tile.getByRole("button", { name: "Selecionar IMG_0420.MOV" }).click();
  ...
  await page.screenshot({ path: testInfo.outputPath("raw-tile-two-deliveries-narrow.png") });
  expect(await page.evaluate(() => document.documentElement.scrollWidth > document.documentElement.clientWidth + 1)).toBe(false);
  await page.setViewportSize({ width: 1280, height: 720 });
  await tile.scrollIntoViewIfNeeded();
  await page.screenshot({ path: testInfo.outputPath("raw-tile-two-deliveries-desktop.png") });
