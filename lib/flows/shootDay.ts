...
export async function createShootDay(input: ShootDayInput): Promise<ShootDayResult> {
  ...
  for (const link of rows.links) await linkTasks(link.parentId, link.childId, link.workflowStepId, "workflow_step");
  if (input.planId) {
    for (const delivery of deliveries) await linkTasks(input.planId, delivery.id, null, "structural_member");
  }
