...
| R1.9 | **`e2e/plan-delivery.spec.ts:108` falha** ("etapa que também é membro de plano mantém os DOIS vínculos após o autosave"). O autosave do modal fica preso em **"Salvando…"** por mais de 20s e nunca chega a "Salvo"; os outros 4 casos do arquivo passam. **O que já foi descartado (2026-09-07):** não é a normalização do diff do autosave (falha igual com o `JSON.stringify` cru de volta), não é o despacho de notificação (falha igual com `notifyTaskChange` desligado na rota), e não é a janela de digitação (falha igual com 700ms). Falha também no HEAD já commitado. **O que falta saber:** se a requisição PATCH chega a completar — a suspeita é o caminho `setTaskPlanLink` num card que é etapa de fluxo E membro de plano ao mesmo tempo, que é exatamente o cenário que este teste existe para cobrir. Não consegui datar a última vez que passou. | esta sessão | Pequeno-médio. Investigar antes de mexer em vínculo de plano. |
